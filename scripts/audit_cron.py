...
def evaluate_risk(expr: str, session_target: str, safety_level: str):
    high_reasons = []
    is_high_freq = cron_is_high_freq(expr)
    is_non_isolated = session_target != "isolated"

    # strict: any non-isolated is high risk
    if safety_level == "strict":
        if is_high_freq and is_non_isolated:
            high_reasons.append("high-frequency-non-isolated")
        if is_non_isolated:
            high_reasons.append("non-isolated")
    # balanced: only non-isolated + high-frequency is high risk
    elif safety_level == "balanced":
        if is_high_freq and is_non_isolated:
            high_reasons.append("high-frequency-non-isolated")
    # relaxed: same as balanced, but we keep severity lower in UI layer
    else:  # relaxed
        if is_high_freq and is_non_isolated:
            high_reasons.append("high-frequency-non-isolated")
...
def main():
    ...
    for j in jobs:
        ...
        high_reasons = evaluate_risk(expr, session_target, safety_level)
